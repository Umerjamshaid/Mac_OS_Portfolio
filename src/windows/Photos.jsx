...
import { ChevronLeft, ChevronRight, X, ZoomIn, Heart, Share2, Trash2, Grid, Rows3 } from "lucide-react";
...
const SIDEBAR_SECTIONS = [
  {
    heading: "Library",
    items: [
      { id: "allPhotos",  label: "All Photos",  icon: "🖼" },
    ...
    <div className="absolute inset-0 z-50 bg-black/95 flex flex-col" onClick={onClose}>
      ...
      >
        ...
        <div className="flex items-center gap-2">
          ...
          <button className="p-1.5 rounded-full hover:bg-white/10 transition-colors text-red-400">
            <Trash2 size={15} />
          </button>
        </div>
      </div>
      ...
      >
        {photos.map((p, i) => (
          <button
            key={p.id}
            onClick={() => {
              if (i < idx) onPrev(idx - i);
              else if (i > idx) onNext(i - idx);
            }}
      ...
      <div className="p-4 space-y-2">
        {photos.map((photo) => (
          ...
          >
            ...
            <ZoomIn size={13} className="text-gray-300 group-hover:text-gray-500 flex-shrink-0 transition-colors" />
          </button>
        ))}
      </div>
    );
    ...
    <div className="relative flex flex-col h-full overflow-hidden bg-white">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="photos-sidebar">
          <p className="text-base font-bold text-gray-800 px-3 mb-2">Photos</p>
        ...
        <div className="flex-1 overflow-y-auto bg-white relative">
          {activeSection === "memories" ? (
            <MemoriesView onPickMemory={handleMemoryPick} />
